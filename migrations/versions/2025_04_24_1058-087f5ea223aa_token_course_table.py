...
def upgrade() -> None:
    ...
    sa.Column('type', sa.Enum('TRAINING', 'PROFESSIONAL_DEVELOPMENT', name='coursetype'), nullable=False),
    sa.Column('creation_at', sa.DateTime(), nullable=False),
    sa.Column('last_update_at', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('title')
    )
    op.create_table('courses_tasks',
...
def downgrade() -> None:
    ...
    sa.Column('phone_number', sa.CHAR(length=11), autoincrement=False, nullable=False),
    ...
    sa.Column('patronymic', sa.VARCHAR(length=30), autoincrement=False, nullable=True),
    sa.Column('role', postgresql.ENUM('ADMIN', 'TRAINER', 'USER', name='role'), autoincrement=False, nullable=False),
    sa.Column('gender', postgresql.ENUM('MALE', 'FEMALE', name='gender'), autoincrement=False, nullable=False),
    sa.Column('date_of_birth', sa.DATE(), server_default=sa.text('CURRENT_DATE'), autoincrement=False, nullable=False),
    sa.Column('avatar_url', sa.VARCHAR(length=40), autoincrement=False, nullable=True),
    sa.Column('creation_at', postgresql.TIMESTAMP(), autoincrement=False, nullable=False),
